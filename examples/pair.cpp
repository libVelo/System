...
{
	public:
		uppercase_filter(stream_protocol::socket sock) : socket_(std::move(sock))
		{
			read();
		}
	...
	private:
		void read()
		{
			socket_.async_read_some(boost::asio::buffer(data_), [this](boost::system::error_code ec, std::size_t size)
			{
				if(!ec)
				{
					// Compute result.
...
int main()
{
	try
	{
		...
		boost::thread thread([&io_context]()
									{
										try
										{
											io_context.run();
